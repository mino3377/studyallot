...
### ゲストログイン
サイトURL : [studyallot.com](https://studyallot.com)
<br>
...
StudyAllotは、やるべきタスクを基準に計画を立てられる学習管理アプリです。学習期間や教材の内容量、周回数などをもとに学習内容を日ごとに割り当て、今日やるべきタスクに迷いをなくし、計画的に学習を進めていくことができます。学習が終わったらその日にやったタスクの量や学習時間、内容を記録することで統計データに反映され、進んでいるという実感を得ながら目標まで伴走します。アプリのアイコンはまさに伴走する蝶をイメージして作成されており、教材に見立てた羽で目標達成まで寄り添うという意味が込められています。
...
- 普段勉強している中で困っている点として、"進捗が見えない"、"継続ができない"、"モチベーションが続かない" という声が多かったです。
  <br>
 → 学習の進捗が見えることと、他の学習者と繋がれるような機能によって、継続やモチベーションにつながるのではないかと考えました。
  
- 学習アプリはスマホを使うという方が多かったです。
    <br>
  → 将来的なPWAやネイティブ化は必要と感じました。
  
- アプリで行き詰った点として、"最初に何をすればいいのか迷った" という声が多かったです。
    <br>
  → 説明が足りていない点や、直感的ではない点、無駄にページ数が多い点を改善する必要があると考えました。
  
- 欲しい追加の機能として、総じて計画時の負担を減らす機能が求められていました。
    <br>
  → 学習時間の記録が必要であることと、計画での入力負担を減らしつつ、ある程度自由に計画を立てられるようにすることをまずは解決しようと考えました。。
...
自己完結型のアプリよりも、他のアプリ間で連携ができた方がより便利になると感じました。例えば、計画を立てた後、普段ユーザーが使っているカレンダーアプリでタスクを閲覧できたほうが、そのユーザーにとっては便利です。
...
開いた時にやる気の出るUIは、最初の掴みとして大切に感じました。個人的なこだわりもあり、最終版ではありきたりなものではなく、個性を前面に出したデザイン作りに取り組んでいこうと思いました。
...
ページを分散させるより、ある程度複数の機能を一つのページにまとめることで、わざわざページ遷移をしなくても一つのページで完結できて使い勝手が良くなると感じました。分散しているほど余計なＤＢとのやり取りも多くなり、重く感じる原因の一つにもなると考えました。
...
今回の声で特に目立っていたのが、"ボタンを押しているのに反応しているのかわからない"、"重く感じる" という声でした。ボタンを押した後、サーバー処理中はisPendingで「保存中」とボタン内の文字を変えたり、isDisabledにしてタップできないようにするという改善が必要でした。
...
初めて使う方が何をしていいか迷わないためにも、丁寧に説明することが必要でした。また説明しなくても伝わるUI作りも同時に進めていく必要があります。
...
## 技術スタック
- フロントエンド Next.js React TailwindCSS shadcn/ui dnd-kit
...
初めて完成したときから大きく2回の変更で今の形に行き着きました。ユーザーがより自由に記録を付けることができればそれだけ多くのケースに対応はできますが、データの構造が複雑になってしまったりデータが膨張しすぎてしまう可能性があることを学び、良い教訓となりました。


### 初期案

教材の内容量×周回数の分だけマスをもって、勉強が終わったらチェックを入れていく形でした。たとえば、10ユニットの教材を3周したい場合には、合計30マスを用意し、1周目の1ユニットが終わったらそれに対応するマスにチェックを入れます。
...
最終的に、曜日ごとの比率と日別の学習記録を組み合わせる方法を採用することにしました。教材側には、各曜日にどれくらい進めるかを10段階で表した7つの要素の配列で持たせ、実際の学習は教材ごとに日単位で記録する形にしました。例えば日曜日は休みの日で0、月曜日は少し多めに勉強したいので4、といった具合に計画を決めて、勉強したらその日学習した内容を記録するという形です。これにより、期間や内容量、周回数を後から変更しても対応しやすくなり、毎日のタスク量を細かく設定する手間も減らせました。
欠点は、日ごとに「今日は何ページから何ページまで進める」といった細かな指示までは行わず、何タスク進めるかのみを確認できるため、その日の具体的な学習内容も決めてほしいという要望には応えられていない点と、計画は曜日ごとなので毎日細かく計画を立てることが難しい点です。ただ、これらは多くの場合看過できるもので、それ以上のメリットが多いと判断し採用しました。
...
ここまでご覧いただきありがとうございます。今回が初めてのアプリ制作でしたが、何度も修正を繰り返していくうちにコードの基本的な理解だけでなく、保守やUXなど多くのことを学べたと実感しております。また、アプリアイコンが決定するまでにも多くの時間を要しましたが、納得するまで一つを突き詰めて理解する姿勢は今後より意識していこうと思っています。まだまだ学びたいことは山ほどありますが、今後も多くの実装を通して腕を磨いていきたいと思っています。ゲストログインから是非アプリを見ていただけると嬉しいです。
