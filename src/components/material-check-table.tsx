...
type Props = {
  ...
  rounds: number
  sections: Section[]
  /** 初期状態： "sectionId:rapNo" -> "YYYY-MM-DD"（保存済みの最新記録） */
...
export default function MaterialCheckTable({
  ...
  return (
    <Card className="gap-3 p-2 md:p-4 space-y-3 w-full">
      {/* ヘッダ行 */}
      ...
      <div className="text-xs text-muted-foreground">
        総セクション × 周回：<span className="font-medium">{sections.length} × {rounds}</span>{" "}
        （保存済み <span className="font-medium">{savedCount}</span> / {totalCells}）
      </div>

      <Separator />

      {/* 表全体の横スクロールを優先（モバイルで収める） */}
      <div className="overflow-x-auto max-h:[60vh] md:max-h-[60vh]">
        <table className="w-full border-separate border-spacing-y-0 md:border-spacing-y-1 min-w-[420px] md:min-w-[760px]">
          <thead>
            <tr>
              <th className="text-left text-xs text-muted-foreground px-1 py-1 md:px-2 md:py-1 w-[160px] md:w-[240px] sticky top-0 bg-background">
                セクション
              </th>
              {Array.from({ length: rounds }).map((_, rIdx) => (
                <th
                  key={rIdx}
                  className="text-center text-xs text-muted-foreground px-1 py-1 md:px-2 md:py-1 sticky top-0 bg-background"
                >
                  {rIdx + 1} 周目
                </th>
              ))}
              ...
              <tr key={s.order} className="bg-background">
                <td className="px-1 py-1 md:px-2 md:py-2 text-sm sticky left-0 bg-background z-10">
                  <div className="w-[100px] md:w-[120px]">
                    <div className="truncate cursor-default" title={s.title}>{s.title}</div>
                    {s.id <= 0 && (
                          ...
                          {isChecked ? (
                            saved ? (
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  ...
                                  <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction onClick={() => { confirmRemoveSaved(key) }}>
                                      外す
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                                </AlertDialogContent>
                              ...
                              <Checkbox
                                checked
                                disabled={s.id <= 0}
                                onCheckedChange={(v) => { if (!v) toggleCell(s.id, rapNo, false) }}
                                className={baseBig + (isToday ? green : gray)}
                              />
                            )
                          ) : (
                            <Checkbox
                              checked={false}
                              disabled={s.id <= 0}
                              onCheckedChange={(v) => { if (v) toggleCell(s.id, rapNo, true) }}
                              className={baseBig}
                            />
                          )}
                        </div>
